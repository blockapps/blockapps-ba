...
const projectsController = {
  create: function (req, res) {
    ...
    co(function* () {
      ...
      const result = yield dapp.createProject(accessToken, projectArgs);
      util.response.status200(res, {project: result});
    }).catch(err => {
  ...
  get: function (req, res) {
    ...
    co(function* () {
      const dapp = yield dappJs.setContract(deploy.admin, deploy.contract);
      const result = yield dapp.getProject(accessToken, projectName);
      util.response.status200(res, {project: result});
    }).catch(err => {
  ...
  list: function (req, res) {
    ...
    co(function* () {
      ...
      util.response.status200(res, {projects: projects});
    }).catch(err => {
  ...
  bid: function (req, res) {
    ...
    co(function* () {
      const dapp = yield dappJs.setContract(deploy.admin, deploy.contract);
      const bid = yield dapp.createBid(accessToken, req.params.name, req.body.supplier, req.body.amount);
      util.response.status200(res, {bid: bid});
    }).catch(err => {
  ...
  getBids: function (req, res) {
    ...
    co(function* () {
      ...
      util.response.status200(res, {bids: bids});
    }).catch(err => {
  ...
  handleEvent: function (req, res) {
    ...
    co(function* () {
      ...
      const result = yield dapp.handleEvent(accessToken, args);
      // got it
